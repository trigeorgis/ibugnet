...
tf.app.flags.DEFINE_string('eval_dir', 'ckpt/eval_svs',
                           '''Directory where to write event logs '''
                           '''and checkpoint.''')
tf.app.flags.DEFINE_string('dataset_dir', '/vol/atlas/databases/body/SupportVectorBody/crop-mpii-train/',
...
def test(output_svs=5, output_lms=16, num_iterations=4):
    ...
    with g.as_default():
        ...
        [images, keypoints_visible, keypoints_visible_mask,
         heatmap, heatmap_mask, gt_landmarks, scales] = provider.get(
            'keypoints_visible/mask','heatmap/mask','landmarks','scale')
    ...
    with tf.Session(graph=g) as sess:
        ...
        summary_ops.append(tf.image_summary('predictions/part-detection', generate_heatmap(part_prediction), max_images=min(FLAGS.batch_size,4)))
        ...
        summary_ops.append(tf.image_summary('mask/visible', keypoints_visible_mask, max_images=min(FLAGS.batch_size,4)))
        ...
        summary_ops.append(tf.image_summary('gt/visiable', generate_landmarks(keypoints_visible), max_images=min(FLAGS.batch_size,4)))
